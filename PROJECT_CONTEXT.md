...
- **태그 카테고리 시스템 추가**
  - `tag_categories` 테이블 및 API (`tagCategoriesApi`)
  - `TagInput` 컴포넌트 개선: 카테고리별 태그 선택 패널
  - `TagManagerModal` 추가: 카테고리/태그 관리 UI (드래그앤드롭으로 태그 이동)
  - 홈 헤더에 태그 관리 버튼 추가
- **TikTok 임베드 지원**: blockquote + embed.js 방식으로 공식 임베드
- **TikTok OG 파싱 개선**: oEmbed API 우선 시도로 썸네일 안정적 확보
- **홈 영상 표시 개선**: 채널에 속한 영상도 전체보기에 표시
- **폴더 카드 섹션 제거**: 탭으로 충분하므로 홈에서 폴더 카드 영역 삭제

### 2026-01-02

- **검색 기능 개선**: 태그로도 검색 가능 (`#태그명` 또는 `태그명`)
- **폴더 탭에 드래그앤드롭**: 영상/채널을 폴더 탭에 직접 드롭 가능
- **영상 → 폴더 이동**: 선택 툴바에서 영상을 폴더로 이동하는 기능 추가
- **폴더 영상 개수 표시**: 폴더 카드/탭에 채널 수 + 영상 수 표시
- **미분류 영상 필터**: "미분류만" 버튼으로 폴더/채널 없는 영상만 보기
